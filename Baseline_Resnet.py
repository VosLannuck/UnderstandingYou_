...
class ResidualBlock(torch.nn.Module):
    def __init__( self, in_channels : int , out_channels: int ,
                  stride : int = 1, downsample : torch.nn.Sequential = None):
...
class BaselineResnet(torch.nn.Module):
    ...
    def __init__(self, residualBlock : ResidualBlock, total_residualBlocks : List[int] = [3, 4, 6, 3], numClasses : int = 2):
        ...
        self.max_pool_1 : MaxPool2d = MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer_1 : torch.nn.Sequential = self._make_layer(residualBlock,64,
                                                               total_residualBlock=total_residualBlocks[0],
                                                               stride=1,
                                                              )
        self.layer_2 : torch.nn.Sequential = self._make_layer(residualBlock, 128,
                                                              total_residualBlock=total_residualBlocks[1],
                                                              stride=2)
        self.layer_3 : torch.nn.Sequential = self._make_layer(residualBlock, 256, 
                                                              total_residualBlock=total_residualBlocks[2],
                                                              stride = 2)
        self.layer_4 : torch.nn.Sequential = self._make_layer(residualBlock, 512, 
                                                              total_residualBlock=total_residualBlocks[3],
                                                              stride = 2
                                                              )
        self.avg_pool : torch.nn.AvgPool2d = torch.nn.AvgPool2d(kernel_size=7, stride=1)
        self.fc : torch.nn.Linear = Linear(512, numClasses)
    def _make_layer(self, residualBlock : ResidualBlock, output_channel : int,
                    total_residualBlock : int, stride : int = 1  ) -> torch.nn.Sequential:
        ...
        listOfLayers.append(residualBlock(self.inChannel, output_channel, stride, downSample))
